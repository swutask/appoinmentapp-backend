...
var AppointmentHelper = function(infxApp) {
    ...
    return {
        ...
        createAppointment: function(args, callback) {
            ...
            /*
            	clientID: ObjectId('59ce27411ca3d91900e9ca75'),
                $and: [
                    {
                        $or: [
                            {
                                start: {
                                    ...
                                    $lt: ISODate("2018-05-01T00:00:00.000Z")
                                }
                            }
                        ]
                    }
            ...
            */
            ...
            // var qry = {
            // 	clientID: args.appointment.clientID,
            //     $and: [
            //         {
            //             $or: [
            //                 {
            //                     start: {
            //                         $gte: new Date(new Date(args.appointment.start).toISOString()),
            //                         $lt:  new Date(new Date(args.appointment.end).toISOString())
            //                     }
            //                 },
            //                 {
            //                 	end: {
            ...
            var qry = {
                calendarID: args.appointment.calendarID,
                $and: [{
                    $or: [{
                            start: new Date(new Date(args.appointment.start).toISOString())
                        },
                        {
                            end: new Date(new Date(args.appointment.end).toISOString())
                        },
                        {
                            start: {
                                $gte: new Date(new Date(args.appointment.start).toISOString()),
            ...
            appointment.find(qry, function(err, ret_appointments) {

                console.log('ret_appointments');
                console.log(ret_appointments);

                if (ret_appointments.length > 0) {

                    return callback(infxApp.helpers.errors.throw.httpNotFound('Appointment can not be created in the following time slot'));
